...
def nicchart(gmin,pmin,pmax,cm=None,cp=None):
...
def drlocus_chart():
    ...
    angle_vect = np.arange(0.1,1.1,0.1)
    for index in range(len(angle_vect)):
        wn = angle_vect[index]*np.pi
        ...
        name = "{:.1f} \pi /T".format(angle_vect[index])
        x = np.real(dpole)
        y = np.imag(dpole)
        data_temp = {"x": x,"y":y,"name":name}
        data.append(data_temp)
    ...
    #add damping line
    m_vect = np.arange(0,1,0.1)

    for index in range(len(m_vect)):
        m = m_vect[index]
        #np.sqrt(1-m**2)*wn_vect
...
def pole_info(pole,dt=None):
    
    if dt is None:
        pole_continuous = pole
    else:
        pole = pole.astype(complex) 
        pole_continuous = np.log(pole)/dt
